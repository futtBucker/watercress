...
//        .style("stroke", function(d) { return color(d.name); });
    ...
    path.on('click', function (d) {
        console.log('klik',d);
        var kec = $('select[name=inputkec').val();
        $.ajax({
            url: base_url + '/barchart2',
            data: {'kecamatan': kec, 'subvar': d.name},
            dataType: "json",
            type: "get",
            success: function (response) {
                generateBarChart2(response);
    ...
    var svg_legend = $('#line-chart-legend');
    svg_legend.html('');

    var line_legend = svg_legend.append("g")
